...
int main (int argc, char **argv)
{
  ...
  SeedManager::SetSeed (167+nodeCount);

  // Set Animation 
  AnimationInterface anim ("wachulero.xml");

  std::mt19937 rng;
  rng.seed(std::random_device()());
  std::uniform_int_distribution<std::mt19937::result_type> dist6(0,nodeCount);
  ...
  for (int i = 0; i < nodeCount; ++i) {
    float x = dist6(rng);
    float y = dist6(rng);
    nodesPositionAlloc->Add (Vector (0.0, x, y));
